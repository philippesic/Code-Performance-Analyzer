...
export class CpaPanelProvider implements vscode.WebviewViewProvider {
    ...
    public updateAnalysisResult(payload: { message?: string; chartData?: any } | string) {
        if (this._view) {
            ...
            if (typeof payload === 'string') {
                ...
                // Handle both {message, chartData} and {text, chartData} formats
                const text = payload.message || (payload as any).text || '';
                this._view.webview.postMessage({
                    command: 'updateResult',
                    text: text,
